...
bool BMP::WriteToFile( const char* FileName )
{
 ...
 int res = fopen_s(&fp, FileName, "wb");
 if(res != 0)
 {
  if( EasyBMPwarnings )
  {
   cout << "EasyBMP Error: Cannot open file " 
        << FileName << " for output: error code "+to_string(res) << endl;
  }
  fclose( fp );
  return false;
 }
...
bool Rescale( BMP& InputImage , char mode, int NewDimension )
{
     ...
     CapMode != 'F' )
 {
  if( EasyBMPwarnings )
  {
   ...
   sprintf_s( ErrorMessage, "EasyBMP Error: Unknown rescale mode %c requested\n" , mode );
   cout << ErrorMessage; 
  }
  return false;
 }
